Feature: Login and Logout Feature for Middle Manager

  
    Scenario: Verify that correct validation message is appearing if admin deactive a mid manager and mid manager enters deactivated mu's email
    ...
    |Your account has been deactivated, please contact your admin.|Your account has been deactivated, please contact your manager.|
  	...
  	Scenario: Verify that correct validation message is appearing if mid manager enters wrong password
    ...
    Scenario: Verify that mid manager is logged in and logged out successfully after entering valid credentials
  	Given Manager is on Admin Home page
    And Manager is logged In
       |UserName|Password|
       |manager_email|manager_pw|
